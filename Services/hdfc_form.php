...
function payment_success() {
	...
	header("Location: online_order_success.php?odi=".$_GET['odi']."&user_id=".$_GET['user_id']."");
}

function payment_failure() {
	/* Payment failure logic goes here. */
	//echo "We are sorry. The Payment has failed";
	header("Location: online_order_failure.php?odi=".$_GET['odi']."&user_id=".$_GET['user_id']."");
}
...
?>
	...
	<form method='POST' name="hdfc_pay_form" style="display:none">
		<input type="text" name="order_id" value="<?php echo $_GET['odi']; ?>">
		...
		<input name='email' type='text' value='srinivas@lanciussolutions.in'>
